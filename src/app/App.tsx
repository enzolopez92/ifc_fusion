...
import { FolderOpenOutlined, CompareArrowsSharp, VerticalShadesTwoTone } from '@mui/icons-material';
...
function App() {
	...
	useEffect(() => {
		if (ifcContainer.current) {
			...
			const ifcViewer = new IfcViewerAPI({ container, backgroundColor: new Color(0xa9a9a9) });

			ifcViewer.axes.setAxes(25)
			ifcViewer.grid.setGrid(80, 80);
		...
		<>
			<Box sx={{ display: 'flex' }}>
				...
				<Drawer variant='permanent' open={isDrawerOpen}>
					...
					{/* Where the buttons are loacted */}
					<List>
						{/* Load IFC File BTN */}
						<input type='file' id='file' accept='.ifc' onChange={ifcOnLoad} style={{ display: 'none' }} />
						<label htmlFor='file'>
						...
						{/* Open a modal with the floors */}
						<ListItemButton key={'showRightPanel'}>
							<ListItemIcon>
								<VerticalShadesTwoTone />
							</ListItemIcon>
							<ListItemText primary={'View Floors'} />
						</ListItemButton>
					</List>
				</Drawer>
			...
			{/* Pop up warning message, success or fail */}
